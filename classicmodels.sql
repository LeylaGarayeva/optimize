...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 17, 2017 at 10:01 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `office_code` (IN `office_code` INT)  NO SQL
select customers.customerName,offices.officeCode From offices JOIN employees on offices.officeCode=employees.officeCode join customers on employees.employeeNumber=customers.salesRepEmployeeNumber WHERE offices.officeCode=1$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sum_amountByCity` (IN `a` VARCHAR(255))  NO SQL
select `classicmodels`.`customers`.`city` AS `city`,sum(`classicmodels`.`payments`.`amount`) AS `sum(payments.amount)` from (`classicmodels`.`customers` join `classicmodels`.`payments` on((`classicmodels`.`customers`.`customerNumber` = `classicmodels`.`payments`.`customerNumber`))) where (`classicmodels`.`customers`.`city` = a)$$
...
--
-- Stand-in structure for view `ordernumber`
--
...
--
-- Stand-in structure for view `sum_amount`
--
CREATE TABLE `sum_amount` (
`city` varchar(50)
,`sum(payments.amount)` decimal(32,2)
);

-- --------------------------------------------------------

--
-- Structure for view `ordernumber`
--
DROP TABLE IF EXISTS `ordernumber`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ordernumber`  AS  select `orderdetails`.`orderNumber` AS `orderNumber`,`products`.`productCode` AS `productCode` from (`orderdetails` join `products` on((`orderdetails`.`productCode` = `products`.`productCode`))) where (`orderdetails`.`orderNumber` = 10100) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `product_stock`  AS  select `products`.`productName` AS `productName`,`products`.`quantityInStock` AS `quantityInStock` from `products` where (`products`.`quantityInStock` < 1000) ;

-- --------------------------------------------------------

--
-- Structure for view `sum_amount`
--
DROP TABLE IF EXISTS `sum_amount`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `sum_amount`  AS  select `customers`.`city` AS `city`,sum(`payments`.`amount`) AS `sum(payments.amount)` from (`customers` join `payments` on((`customers`.`customerNumber` = `payments`.`customerNumber`))) where (`customers`.`city` = 'Nantes') ;
